...
export const getAreaLayersCartogram = (layers, setStatistics, grouping) => {
    ...
    layers.features.map((item, index) => {
        ...
        if (graphStatics.find((area) => area[0] === item.properties.description)) {
            graphStatics.map((area) => {
                if (area[0] === item.properties.description) {
                    area[1] = +parseFloat(
                        area[1] + +parseFloat(item.properties.area).toFixed(1)
                    ).toFixed(1);
                }
...
export const getColorLayersCartogram = (layers, setColorLayers, grouping) => {
    let colorsLayers = [];
    let nameParams = grouping
